...
export function validateQuestionAccess(attempt, requestedQuestionIndex) {
  ...
  // Allow access to previous or current questions
  ...
  // Prevent skipping ahead beyond currentQuestionIndex + 1
  if (requestedQuestionIndex > attempt.currentQuestionIndex + 1) {
    return {
      valid: false,
      error: 'Cannot skip ahead. Access is limited to current or previous questions.',
...
export function validateAnswerSubmission(attempt, questionIndex) {
  ...
  // Allow answering previous or current questions; block skipping ahead
