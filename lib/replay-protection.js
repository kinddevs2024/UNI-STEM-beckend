...
const MIN_ANSWER_TIME_MS = 0; // Minimum answer time disabled to avoid false positives
...
export function issueQuestionNonce(attempt, questionId) {
  ...
  const existingNonceData = attempt.questionNonces[questionId];
  if (existingNonceData) {
    const existingExpiresAt = new Date(existingNonceData.expiresAt);
    const isExpired = now > existingExpiresAt;
    const isUsed = Boolean(existingNonceData.used);

    if (!isExpired && !isUsed) {
      return {
        nonce: existingNonceData.nonce,
        issuedAt: existingNonceData.issuedAt,
        expiresAt: existingNonceData.expiresAt
      };
    }
  }
  ...
  if (typeof attempt.markModified === 'function') {
    attempt.markModified('questionNonces');
  }
