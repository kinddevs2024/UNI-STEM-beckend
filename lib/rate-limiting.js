...
const RATE_LIMITS = {
  ...
  '/auth/login': { windowMs: 60000, maxRequests: 10 }, // 10 per minute per IP
  '/auth/register': { windowMs: 60000, maxRequests: 5 }, // 5 per minute per IP
  '/upload': { windowMs: 60000, maxRequests: 20 }, // 20 uploads per minute per IP
};
...
/**
 * Check rate limit by IP only (for auth, upload endpoints before user context)
 * @param {String} endpoint - Endpoint identifier (e.g. '/auth/login')
 * @param {Object} req - Request object with headers, connection
 * @returns {Object} - { allowed, remaining, resetAt, limit }
 */
export function checkRateLimitByIP(endpoint, req) {
  // Use a single shared key for all IPs to avoid per-IP throttling.
