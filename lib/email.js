...
function createTransport() {
  ...
  console.log('[Nodemailer] Creating transport with config:', {
    host: cfg.host,
    port: cfg.port,
    secure: cfg.secure,
    user: cfg.user,
...
export async function sendPasswordResetCodeEmail({ to, name, code }) {
  ...
  console.log('[sendPasswordResetCodeEmail] Sending to:', to, 'with code:', code);
  try {
    ...
    console.log('[sendPasswordResetCodeEmail] Success:', result.messageId);
    return result;
  } catch (error) {
    console.error('[sendPasswordResetCodeEmail] Failed:', {
      to,
      error: error.message,
      code: error.code,
...
export async function sendEmailVerification({ to, name, link, code }) {
  ...
  console.log('[sendEmailVerification] Sending to:', to, 'with code:', code);
  try {
    const result = await transporter.sendMail({
      from: cfg.from,
      to,
    ...
    console.log('[sendEmailVerification] Success:', result.messageId);
    return result;
  } catch (error) {
