// CORS middleware helper for Next.js API routes
import { getSystemControlsSync } from '../system-controls.js';

const DEV_ORIGINS = ['http://localhost:5173', 'http://localhost:3000'];

function shouldBypassApiKillSwitch(req) {
  const url = String(req.url || '');
  return url.includes('/api/owner/system-controls');
}

function toOrigin(value) {
  if (!value || typeof value !== 'string') return null;
  try {
    return new URL(value).origin;
  } catch {
    return null;
  }
}

function toHostname(value) {
  if (!value || typeof value !== 'string') return null;
  try {
    return new URL(value).hostname;
  } catch {
    return null;
  }
}

function withWwwVariants(originValue) {
  const normalizedOrigin = toOrigin(originValue);
  if (!normalizedOrigin) return [];

  try {
    const parsed = new URL(normalizedOrigin);
    const host = parsed.hostname;
    const variants = new Set([normalizedOrigin]);

    if (host.startsWith('www.')) {
      parsed.hostname = host.slice(4);
      variants.add(parsed.origin);
    } else {
      parsed.hostname = `www.${host}`;
      variants.add(parsed.origin);
    }

    return [...variants];
  } catch {
    return [normalizedOrigin];
  }
}

function parseForwardedProto(req) {
  const forwarded = req.headers['x-forwarded-proto'];
  if (!forwarded || typeof forwarded !== 'string') {
    return process.env.NODE_ENV === 'production' ? 'https' : 'http';
  }
  return forwarded.split(',')[0].trim() || 'http';
}

function getAllowedOrigins(req) {
  const allowed = new Set();

  const configured = (process.env.ALLOWED_ORIGINS || '')
    .split(',')
    .map((entry) => toOrigin(entry.trim()))
    .filter(Boolean);

  configured.forEach((entry) => {
    withWwwVariants(entry).forEach((origin) => allowed.add(origin));
  });

  const frontendOrigin = toOrigin(process.env.FRONTEND_URL);
  if (frontendOrigin) {
    withWwwVariants(frontendOrigin).forEach((origin) => allowed.add(origin));
  }

  const forwardedHost = req.headers['x-forwarded-host'] || req.headers.host;
  if (forwardedHost && typeof forwardedHost === 'string') {
    const serverOrigin = `${parseForwardedProto(req)}://${forwardedHost}`;
    const normalized = toOrigin(serverOrigin);
    if (normalized) {
      allowed.add(normalized);
    }
  }

  if (process.env.NODE_ENV !== 'production') {
    DEV_ORIGINS.forEach((origin) => allowed.add(origin));
  }

  return [...allowed];
}

function isSameRequestHost(req, requestOrigin) {
  const originHostname = toHostname(requestOrigin);
  if (!originHostname) return false;

  const forwardedHostRaw = req.headers['x-forwarded-host'];
  const hostRaw = req.headers.host;

  const forwardedHost = typeof forwardedHostRaw === 'string'
    ? forwardedHostRaw.split(',')[0].trim()
    : null;
  const directHost = typeof hostRaw === 'string' ? hostRaw.trim() : null;

  const hostCandidates = [forwardedHost, directHost].filter(Boolean);

  return hostCandidates.some((candidate) => {
    const candidateHostname = candidate.split(':')[0].trim().toLowerCase();
    return candidateHostname && candidateHostname === originHostname.toLowerCase();
  });
}

export function handleCORS(req, res) {
  if (!shouldBypassApiKillSwitch(req)) {
    const controls = getSystemControlsSync();
    if (controls.apiEnabled === false) {
      if (req.method === 'OPTIONS') {
        res.status(503).end();
        return true;
      }

      res.status(503).json({
        success: false,
        message: 'API is temporarily disabled by owner control',
        code: 'API_DISABLED',
      });
      return true;
    }
  }

  const requestOrigin = toOrigin(req.headers.origin);
  const allowedOrigins = getAllowedOrigins(req);
  const originAllowed =
    !requestOrigin ||
    allowedOrigins.includes('*') ||
    allowedOrigins.includes(requestOrigin) ||
    isSameRequestHost(req, requestOrigin);

  if (!originAllowed) {
    if (req.method === 'OPTIONS') {
      res.status(403).end();
      return true;
    }
    res.status(403).json({ message: 'CORS origin denied' });
    return true;
  }

  if (requestOrigin) {
    res.setHeader('Access-Control-Allow-Origin', requestOrigin);
  }

  res.setHeader('Access-Control-Allow-Credentials', 'true');
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, PATCH, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With');
  res.setHeader('Vary', 'Origin');

  if (req.method === 'OPTIONS') {
    res.setHeader('Access-Control-Max-Age', '86400');
    res.status(200).end();
    return true;
  }

  return false;
}

