...
import { getUploadBaseDir } from "./upload-path.js";
...
export const parseForm = (req, uploadDir = "./uploads", options = {}) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const uploadPath = getUploadBaseDir(uploadDir);

    // Check disk space before upload (logs warning if <10% free)
    checkUploadDiskSpace(uploadPath);
 ...
 */
function getStructuredPath(userId, originalFilename, extension) {
  const now = new Date();
...
export const saveFile = async (file, destination, userId = null) => {
  ...
  const relativePath = getStructuredPath(userId, originalFileName, finalExtension);
  const baseUploadsPath = getUploadBaseDir(destination);
  const uploadPath = path.join(baseUploadsPath, path.dirname(relativePath));
  const filePath = path.join(baseUploadsPath, relativePath);
