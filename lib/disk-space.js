...
export function getDiskSpaceInfo(dirPath) {
  ...
  if (process.platform === "win32") {
    try {
      const drive = path.parse(resolvedPath).root.replace(/\\+$/, "");
      if (drive) {
        const output = execSync(
          `powershell -NoProfile -Command "$d = Get-PSDrive -Name '${drive[0]}'; $o = [pscustomobject]@{ Free = $d.Free; Used = $d.Used }; $o | ConvertTo-Json -Compress"`,
